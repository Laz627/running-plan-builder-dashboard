'use client';

import Modal from '@/components/Modal';
import { LIFT_EXERCISES } from '@/lib/exercises';
import { useState } from 'react';
import { toast } from '@/components/Toaster';

export type LiftLog = {
  id: number;
  logDate: string;
  dayType?: string | null;
  exercise?: string | null;
  weight?: number | null;
  sets?: number | null;
  reps?: number | null;
  rpe?: number | null;
  notes?: string | null;
};

function isoDate(s?: string) {
  if (!s) return '';
  return new Date(s).toISOString().slice(0, 10);
}

export default function EditLiftModal({
  open,
  onClose,
  entry,
  onSaved,
  onDeleted,
}: {
  open: boolean;
  onClose: () => void;
  entry: LiftLog | null;
  onSaved: () => void;
  onDeleted: () => void;
}) {
  const [form, setForm] = useState<LiftLog | null>(entry);

  // resync on selection change
  // eslint-disable-next-line react-hooks/exhaustive-deps
  if (open && entry && form?.id !== entry.id) setForm(entry);

  function set<K extends keyof LiftLog>(k: K, v: any) {
    if (!form) return;
    setForm({ ...form, [k]: v });
  }

  async function save() {
    if (!form) return;
    const ok = await fetch(`/api/logs/lift/${form.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(form),
    }).then(r => r.ok).catch(() => false);
    if (ok) {
      toast({ title: 'Lift updated', description: 'Changes saved.' });
      onSaved();
      onClose();
    } else {
      toast({ title: 'Error', description: 'Could not save changes.' });
    }
  }

  async function remove() {
    if (!form) return;
    const ok = await fetch(`/api/logs/lift/${form.id}`, { method: 'DELETE' })
      .then(r => r.ok).catch(() => false);
    if (ok) {
      toast({ title: 'Lift deleted', description: 'Entry removed.' });
      onDeleted();
      onClose();
    } else {
      toast({ title: 'Error', description: 'Could not delete lift.' });
    }
  }

  return (
    <Modal open={open} onClose={onClose} title="Edit Lift" maxWidthClass="max-w-xl">
      {!form ? (
        <div className="text-sm text-gray-500">No entry selected.</div>
      ) : (
        <>
          <div className="grid gap-3 sm:grid-cols-3">
            <label className="text-sm">
              Date
              <input type="date" className="input"
                value={isoDate(form.logDate)}
                onChange={e => set('logDate', e.target.value)} />
            </label>
            <label className="text-sm">
              Day
              <select className="input" value={form.dayType || 'Push'}
                onChange={e => set('dayType', e.target.value)}>
                <option>Push</option><option>Pull</option><option>Legs</option>
              </select>
            </label>
            <label className="text-sm">
              Exercise
              <select className="input" value={form.exercise || LIFT_EXERCISES[0]}
                onChange={e => set('exercise', e.target.value)}>
                {LIFT_EXERCISES.map(ex => <option key={ex} value={ex}>{ex}</option>)}
              </select>
            </label>
            <label className="text-sm">
              Weight (lb) / Assist
              <input type="number" className="input"
                value={form.weight ?? 0}
                onChange={e => set('weight', parseFloat(e.target.value || '0'))}/>
            </label>
            <label className="text-sm">
              Sets
              <input type="number" className="input"
                value={form.sets ?? 3}
                onChange={e => set('sets', parseInt(e.target.value || '3', 10))}/>
            </label>
            <label className="text-sm">
              Reps
              <input type="number" className="input"
                value={form.reps ?? 8}
                onChange={e => set('reps', parseInt(e.target.value || '8', 10))}/>
            </label>
            <label className="text-sm">
              RPE (1–10)
              <input type="number" className="input" min={1} max={10}
                value={form.rpe ?? 7}
                onChange={e => set('rpe', parseInt(e.target.value || '7', 10))}/>
            </label>
            <label className="text-sm sm:col-span-3">
              Notes
              <textarea className="input" rows={3}
                value={form.notes || ''} onChange={e => set('notes', e.target.value)} />
            </label>
          </div>

          <div className="mt-4 flex gap-2 justify-between">
            <button className="btn" onClick={remove}>Delete</button>
            <div className="flex gap-2">
              <button className="btn" onClick={onClose}>Cancel</button>
              <button className="btn" onClick={save}>Save</button>
            </div>
          </div>
        </>
      )}
    </Modal>
  );
}
